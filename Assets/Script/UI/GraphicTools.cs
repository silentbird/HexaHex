...
namespace pure.ui.imageFiller {
	public static class GraphicTools {
		...
		public static Vector2 CaculateBezier(float t, Vector2 v0, Vector2 v1, Vector2 control) {
			...
			float num2 = t * t;
			return new Vector2((float)((double)v0.x * (double)num1 + (double)control.x * (double)t * (1.0 - (double)t) * 2.0 + (double)num2 * (double)v1.x),
				(float)((double)v0.y * (double)num1 + (double)control.y * (double)t * (1.0 - (double)t) * 2.0 + (double)num2 * (double)v1.y));
		}
		...
		public static Vector4 GetDrawingDimensions(
			...
			Vector4 drawingDimensions = new Vector4(vector4.x / (float)num1, vector4.y / (float)num2, ((float)num1 - vector4.z) / (float)num1,
				((float)num2 - vector4.w) / (float)num2);
		...
		public static Vector4 GetMaskDimensions(
			...
			int num1 = Mathf.RoundToInt(vector2_2.x);
			int num2 = Mathf.RoundToInt(vector2_2.y);
			Vector4 maskDimensions = new Vector4((vector4.x + mask.x) / (float)num1, (vector4.y + mask.y) / (float)num2, ((float)num1 - vector4.z) / (float)num1,
				((float)num2 - vector4.w) / (float)num2);
			RectTransform rectTransform = graphic.rectTransform;
			if (shouldPreserveAspect && (double)vector2_2.sqrMagnitude > 0.0) {
				float num3 = vector2_2.x / vector2_2.y;
